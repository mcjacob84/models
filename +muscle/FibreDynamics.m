classdef FibreDynamics < dscomponents.ACompEvalCoreFun
    ...
    methods
        ...
        function dy = evaluate(this, y, t, mu)
            ...
            % todo anpassen f√ºr matrix-wertiges y, also 58Nx1000 -> 58 x 1000N und zur√ºck
            SarcoRates=this.SarcomerRates(reshape(y(dm+ds+1:end,:),dsa,N)', t, mu);
            % SarcoRates =
            % this.SarcomerRates(reshape(y(dm+ds+1:end,:,dsa,[])), t, mu);
...
%             end
        ...
        function dy = SarcomerRates(this, y, t, mu)
            ...
            a=0; b=1; % urspr¸nglich in initSarcoConsts() in Sarco Modell
                      % a = velocity, welche? ist die in Zustand y?
                            % used in line ~412
                      % b in mu ¸bergeben?   used in line ~260 and ~268
    ...
    methods(Access=private)
        ...
        function initSarcoConst(this)
            ...
            scale_param = 0.1;
            %    scale_param = 1.0;
            % Parameter von Thomas, die urspr√ºnglich mit √ºbergeben wurden.
