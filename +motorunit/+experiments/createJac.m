...
alg(48) =  (1/7)*((exp((y(8)/67.3000))) - 1);
alg(49) = (1+ 0.120000*(exp((-0.1*y(1)*(c(7)/c36c37))))+ 0.04*alg(48)*(exp((- (y(1)*(c(7)/c36c37))))))^-1;
...
dy(55) = c(95)*((y(54) - y(55))/c(105)) - h73*c(96)*(alg(72))*0.001*y(55)*y(32) - h74*c(97)*y(56)*alg(72);
dy(56) = h73*c(96)*alg(72)*0.001*y(55)*y(32) + h74*c(97)*y(56)*alg(72);
       
%% Create partial derivatives
dir = fileparts(which(mfilename));
JP = sparse(false(56,56));
f = fopen(fullfile(dir,'Jac_Sarco_gen.m'),'w+');
fprintf(f,'J = sparse(56,56);\n');
pi = ProcessIndicator('Generating %dx%d Jacobian file',56^2,false,56,56);
for i = 1:56
    for j = 1:56
        pd = diff(dy(i),y(j));
        if pd ~= 0
...
save(fullfile(dir,'JSparsityPattern'),'JP');