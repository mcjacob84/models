...
% - \c License @ref licensing
    ...
    methods
        ...
        function prepareConstants(this, ~)
            ...
            f = sparse(7 * data.num_knots, 1); %#ok<*PROP>
            
%             % Kopplung von T an u (berechnet die Temperaturkräfte)
%             M_T = sparse(7 * data.num_knots, 7 * data.num_knots);

%             % Kopplung von d/dt u an T (Wärmequellen durch Bewegung)
%             K_u = sparse(7 * data.num_knots, 7 * data.num_knots);

            for i = 1:data.num_elem_RO

%                 if linear
%                     [M_lok, K_lok, f_lok] = loc_matrix_lin(RO(i).l, RO(i).T, RO(i).c, RO(i).q_lok);
%                     index_0_lok = [1 3 5 7 9 11];
%                     index_l_lok = [2 4 6 8 10 12];
%                 else
...
%                 [M_lok, K_lok, f_lok] = loc_matrix_heat(RO(i).l, [0; 0]);
%                 index_glob = [7*data.knot_index(RO(i).p(1)) 7*data.knot_index(RO(i).p(2))];
%                 M(index_glob, index_glob) = M(index_glob, index_glob) + RO(i).c_theta * M_lok;
%                 K(index_glob, index_glob) = K(index_glob, index_glob) + RO(i).kappa * K_lok;
...
%                 % Beiträge der Heat-equation assemblieren
%                 [M_lok, K_lok, f_lok] = loc_matrix_heat(KR(i).R * KR(i).angle, [0; 0]);
%                 index_glob = [7*data.knot_index(KR(i).p(1)) 7*data.knot_index(KR(i).p(2))];
%                 if (dynamic)
%                     M(index_glob, index_glob) = M(index_glob, index_glob) + KR(i).c_theta * M_lok;
%                 end
%                 K(index_glob, index_glob) = K(index_glob, index_glob) + KR(i).kappa * K_lok;
            %     f(index_glob) = f(index_glob) + f_lok;
            end
...
%                 K(index_glob, index_glob) = K(index_glob, index_glob) + FH(i).kappa * K_lok;    
            ...
            % Kopplungsmatrix: Auftretende Kräfte auf Grund von Temperatur berechnen
%             for i = 1:data.num_elem_RO
%                 M_T_lok = loc_heat_load_beam(RO(i).T, RO(i).alphaA);
% 
%                 index_0_glob_u = 7*data.knot_index(RO(i).p(1))-6 : 7*data.knot_index(RO(i).p(1))-4;
%                 index_0_glob_T = 7*data.knot_index(RO(i).p(1));
%                 index_l_glob_u = 7*data.knot_index(RO(i).p(2))-6 : 7*data.knot_index(RO(i).p(2))-4;
%                 index_l_glob_T = 7*data.knot_index(RO(i).p(2));
%                 index_glob_u = [index_0_glob_u index_l_glob_u];
%                 index_glob_T = [index_0_glob_T index_l_glob_T];
% 
%                 M_T(index_glob_u, index_glob_T) = M_T(index_glob_u, index_glob_T) + M_T_lok;
%             end
%             for i = 1:data.num_elem_KR
%                 M_T_lok = loc_heat_load_circle(KR(i).R, KR(i).angle, KR(i).T_block1, KR(i).T_block2, KR(i).B3, KR(i).alphaA);
% 
%                 index_0_glob_u = 7*data.knot_index(KR(i).p(1))-6 : 7*data.knot_index(KR(i).p(1))-1;
%                 index_0_glob_T = 7*data.knot_index(KR(i).p(1));
%                 index_l_glob_u = 7*data.knot_index(KR(i).p(2))-6 : 7*data.knot_index(KR(i).p(2))-1;
%                 index_l_glob_T = 7*data.knot_index(KR(i).p(2));
%                 index_glob_u = [index_0_glob_u index_l_glob_u];
%                 index_glob_T = [index_0_glob_T index_l_glob_T];
% 
%                 M_T(index_glob_u, index_glob_T) = M_T(index_glob_u, index_glob_T) + M_T_lok;
...
% 
%             if (dynamic)
%                 % Kopplungsmatrix: Auftretende Wärmequellen auf Grund von Geschwindigkeit berechnen
%                 for i = 1:data.num_elem_RO
%                     K_u_lok = loc_heat_source_beam(RO(i).T, RO(i).alphaA);
% 
%                     index_0_glob_u = 7*data.knot_index(RO(i).p(1))-6 : 7*data.knot_index(RO(i).p(1))-4;
%                     index_0_glob_T = 7*data.knot_index(RO(i).p(1));
%                     index_l_glob_u = 7*data.knot_index(RO(i).p(2))-6 : 7*data.knot_index(RO(i).p(2))-4;
...
%                     index_glob_T = [index_0_glob_T index_l_glob_T];
% 
%                     K_u(index_glob_T, index_glob_u) = K_u(index_glob_T, index_glob_u) + K_u_lok;
%                 end
%                 for i = 1:data.num_elem_KR
%                     K_u_lok = loc_heat_source_circle(KR(i).R*KR(i).angle, KR(i).T_block1, KR(i).T_block2, KR(i).B3, KR(i).alphaA);
% 
%                     index_0_glob_u = 7*data.knot_index(KR(i).p(1))-6 : 7*data.knot_index(KR(i).p(1))-1;
%                     index_0_glob_T = 7*data.knot_index(KR(i).p(1));
%                     index_l_glob_u = 7*data.knot_index(KR(i).p(2))-6 : 7*data.knot_index(KR(i).p(2))-1;
...
%             end
            ...
            dir_u = setdiff(nodes_u, free_u);
            dir_T = setdiff(nodes_T, free_T);
