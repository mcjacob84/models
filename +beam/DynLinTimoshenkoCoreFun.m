...
% - \c License @ref licensing
    ...
    methods
        function this = DynLinTimoshenkoCoreFun(sys)
            ...
            this.sys = sys;
            % DIRTY
            this.x_big_F = ones(this.sys.Model.dim, 1);
        end
        ...
        function fx = evaluateCoreFun(this, x, ~, ~)
            ...
            fx = zeros(size(x));
            if m.isLinear
                fx(this.free_uv) = this.A_big*(this.f_big - this.B_big*x(this.free_uv));
            else
                % Daten nicht aktuell
                % AUSRECHNEN!
                if (norm(this.x_big_F - x) > 1e-8)
                    [this.R_F, this.K_F] = this.weak_form(m.data, m.RO, m.KR, m.FH, x);
                    
                    R = this.R_F(this.free_u);
                    K = this.K_F(this.free_u, this.free_u);
                    
                    this.R_big = [0*R; R];
                    
                    s = 14 * m.data.num_knots;
                    this.J_big_F = zeros(s,s);
                    
                    this.J_big_F (this.free_uv, this.free_uv) = -1 * (this.B_big + [zeros(size(K)), zeros(size(K)); this.K, zeros(size(K))]);
                    this.x_big_F = x;
                end
                
                % Funktion f auswerten
                fx(this.free_uv) = this.A_big * (this.f_big - this.B_big*x(this.free_uv) - this.R_big);
...
%                 K(index_glob, index_glob) = K(index_glob, index_glob) + FH(i).kappa * K_lok;    
            ...
            this.A_big = inv(blkdiag(eye(length(free_u), length(free_u)), M(free_u, free_u)));
            
            if isLinear
                this.B_big = [zeros(length(free_u), length(free_u)), -eye(length(free_u), length(free_u)); K(free_u, free_u), this.C_F(free_u, free_u)];
            else
                this.B_big = [zeros(length(free_u), length(free_u)), -eye(length(free_u), length(free_u)); 0*K(free_u, free_u), this.C_F(free_u, free_u)];
            end
            ...
            if isLinear
                ...
                J = zeros(s,s);
                J(this.free_uv, this.free_uv) = -this.A_big*this.B_big;
                this.J_big_F = J;
            end
